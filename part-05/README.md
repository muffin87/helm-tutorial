...
In this part we will take a look what we can do with an already running release.
It's a common use case to manipulate a running release without deleting and
redeploying it. That would of course cause downtime of our service. With
Helm you can update and also rollback releases. It makes use of the normal
Kubernetes strategies (canary, rolling updates).

Let's start by updating your blog.

## Update

As example we choose a very simple update. We want to update the version of 
...
Kubernetes will handel the update for us. This means it will terminate the Pod
...
That looks very good. You could now try to access the blog again to see if the
change didn't break anything. 

_Note: The canary or the rolling update for a Deployment or a Statefulset will
not be triggered by every value you change in the manifest files. For example
when you change only a label your Pods will not be recreated automatically. 
You would have to trigger the update manually by deleting the corresponding Pods._

So we did an update. But imaging you have an update which is causing trouble and
...
information how Helm managed the state of releases.

## What's under the hood?

So what magic does Helm to keep track of the releases. We can once again list
the helm release with:
...
release such as the name and the version. We should now test if these information
let us rollback to the first revision.
...
So that worked. We did successfully rolled back our release. Let's clean up
the your blog.
